...
export const DeanonymizationPage: React.FC<DeanonymizationPageProps> = ({
  ...
  return (
    <div className="p-8 space-y-8 max-w-7xl mx-auto">
      {/* Top Banner with Problem Statement Attribution (Apple Style) */}
      <div className="bg-gradient-to-br from-white via-blue-50/50 to-slate-50 border border-slate-200/90 rounded-3xl p-8 shadow-sm relative overflow-hidden">
      ...
      {loading && !pipelineData ? (
        ...
        <>
          {/* Target Assessment 4 Metric Cards (Horizontal Layout) */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-white border border-slate-200 rounded-3xl p-5 flex items-center justify-between shadow-2xs">
          ...
          {/* 5-Stage Progressive De-anonymization Ladder (Horizontal Step Timeline) */}
                ...
                <div className="space-y-3 pt-2">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    {(pipelineData.stages[activeStageIndex].evidence_items || pipelineData.stages[activeStageIndex].indicators || []).map((ind: any, i: number) => (
                      <div
                        key={i}
                        className="bg-white border border-slate-200/90 hover:border-blue-300 rounded-2xl p-4 transition-all space-y-2 shadow-2xs"
                      >
          ...
          {/* OPSEC Vulnerability Analysis Panel (Horizontal Cards) */}
          <div className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {(pipelineData.opsec_vulnerabilities || []).map((v: any, idx: number) => (
                <div
                  key={idx}
                  className="bg-white border border-slate-200 hover:border-blue-400 rounded-2xl p-5 transition-all shadow-2xs space-y-3"
                >
          ...
          {/* Attribution Explanation & Methodology Notice (Apple Plain English) */}
          <div className="p-6 bg-blue-50/60 border border-blue-200 rounded-3xl text-xs text-slate-700 space-y-2">
